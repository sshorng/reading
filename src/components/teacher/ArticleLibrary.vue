          ...
          <table class="min-w-full divide-y divide-slate-100 text-left">
            <thead class="bg-slate-50/80">
              <tr>
                ...
                <th class="px-4 py-3 text-xs font-black text-slate-400 uppercase tracking-widest">篇章</th>
                <th class="px-3 py-3 text-xs font-black text-slate-400 uppercase tracking-widest">標籤</th>
                <th class="px-3 py-3 text-xs font-black text-slate-400 uppercase tracking-widest">狀態</th>
                <th class="px-3 py-3 text-right text-xs font-black text-slate-400 uppercase tracking-widest">操作</th>
              </tr>
            </thead>
            <tbody class="bg-white divide-y divide-slate-50">
              ...
              <tr v-for="article in filteredArticles" :key="article.id" class="hover:bg-red-50/30 transition-all group/row">
                ...
                <td class="px-4 py-4">
                  <div @click="viewSubmissions(article)" class="text-base font-black text-slate-800 cursor-pointer hover:text-red-800 transition-colors underline decoration-dotted decoration-red-800/20 underline-offset-4">{{ article.title }}</div>
                  <div class="flex items-center gap-3 text-[10px] text-slate-400 mt-1.5 font-bold uppercase tracking-tight">
                    ...
                    <span v-if="article.deadline" class="text-red-700 font-black border-l pl-3 border-slate-200 flex items-center gap-1">
                      <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                      {{ formatDate(article.deadline) }}
                    </span>
                  </div>
                </td>
                <td class="px-3 py-4">
                ...
                <td class="px-3 py-4 text-right">
                  <div class="flex justify-end gap-1.5">
                    <button @click="editArticle(article)" class="p-2 text-slate-500 hover:text-red-800 hover:bg-red-50 rounded-lg border border-slate-200 transition-all" title="修訂">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                    </button>
                    <button @click="deleteArticle(article.id)" class="p-2 text-slate-300 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all" title="焚毀">
