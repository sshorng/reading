...
/**
 * 💡 AI 靈感發想（PISA 素養導向與品學堂風格，強制破題法）
 */
export async function generateTopicIdea(topic, tags) {
    const formatReq = tags?.format ? `，文章形式必須包含「${tags.format}」` : '';
    const contentTypeReq = tags?.contentType ? `，文體必須是「${tags.contentType}」` : '';
    const difficultyReq = tags?.difficulty ? `，難度約在「${tags.difficulty}」` : '';

    const prompt = `你是一位深諳 PISA 閱讀素養精神、專擅解析現代社會現象與青少年次文化的「大師級閱讀題庫命題員」（風格對標：台灣『品學堂Wisdomhall』）。
你的任務是發想一個極具當代思辨價值、從微小現象看見宏大議題、令國中生耳目一新的「閱讀測驗寫作架構 Prompt」。

主軸方向：${topic ? `「${topic}」` : '請發揮創意，設計一個具備 PISA 素養深度的現代社會現象或次文化解析主題。'}

**【一、選材五大素養抽屜框架 (PISA-Style Expansion)】**
請你**隨機開啟以下其中一個抽屜**，從中發想一個具體且高度貼近現代學生視野的問題意識：
1. **當代媒體識讀**：例如「內容農場的標題殺人法與眼球經濟」、「AI 生成依賴下的思想寄生」、「假新聞與迷因傳播的底層邏輯」。
2. **次文化與流行解構**：例如「萬物皆可『萌』（擬人化）的心理學」、「動畫角色配色與形狀的潛意識暗示」、「同人二次創作的版權灰色地帶」。
3. **跨國人文觀察**：例如「為何日本影劇裡常吃『冷便當』？」、「北歐人擁抱大自然的『Friluftsliv』生活哲學」、「不同文化對『排隊』的成因與解讀」。
4. **社會爭議思辨**：例如「為倡議環保而對名畫潑熱湯，是覺醒還是破壞？」、「博愛座的原本用意與當代年齡焦慮」、「為了動物權利而存在的『無肉星期一』」。
5. **創意文學重構**：將古代人物丟進現代社會。例如「當陶淵明走在炎熱的台北盆地」、「如果李白成為了現代的百萬YouTuber」。

**【二、破除死板與獵奇的鐵律 (絕不暴雷開局法)】**
🚨 **絕對禁令一**：嚴禁任何「死亡、喪葬、驚悚、戰爭、靈異」等邊緣獵奇題材。
🚨 **絕對禁令二**：你的回覆中，【絕對不允許】出現「主題鎖定為...」、「主題選定為...」、「想像一下...」這些死板的機器人報告句型。
請直接用「一句引發好奇心、具備洞察力的爆款疑問句或情境」做開頭！
❌ 錯誤範例：「主題選定『媒體識讀』，探討內容農場。請以...」
✅ 正確範例：「為什麼我們明知道『震驚！15張靈異照片』是假的，手指卻還是忍不住點進去？這是大腦多巴胺與『內容農場』的一場角力戰...」

${formatReq}${contentTypeReq}${difficultyReq}。

**【三、指定輸出格式】**
請直接回傳一份針對「寫手 AI」的指令框架（純文字，不需問候語、Markdown 語法或程式碼塊）。
請用激發寫作慾望、深邃又生動的講述口吻，並以下列三個標題條列輸出：

現象破題與思辨核心：(第一句話直接是勾引學生的問句或情境，接著簡述背後有趣的社會學/心理學/文化真相)
敘事基調與讀者視角：(帶領讀者看穿表象的風格，例如：犀利除魅、跨文化觀察家、時空旅人對話等)
文章結構與邏輯推演：(如何鋪陳現象起因、帶入反思、最終產生洞見的段落節奏)
    `;
