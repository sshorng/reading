...
【嚴格禁止】：絕對不要生成「網路論壇」、「留言板」、「Q&A問答」這類形式，因為已經出太多次了。
請從以下【會考常見題材與文本形式】中，**隨機且跳躍式地挑選**（絕對不要重複常見的今昔對比或夾敘夾議）：
1. **生活應用類 (應用文/圖表)**：例如：產品使用說明書、活動海報、車班時刻表與購票須知、商品廣告文案、博物館導覽摺頁、餐廳特色菜單、圖書館借閱規定。
2. **科學與紀實類 (說明文)**：例如：動物行為觀察報告、氣象與星象觀測紀錄、醫學與健康常見迷思解析、歷史檔案解密、地理景觀成因剖析、考古發掘日誌。
3. **文學與哲理 (記敘/抒情/議論)**：例如：翻譯小說的片段對比、現代詩歌與流行歌詞的賞析、人物傳記的某個小故事、生活中的微小哲學發現、針對某個社會現象的專欄影評。
${formatReq}${contentTypeReq}${difficultyReq}。

請直接回傳一份針對「寫手 AI」的指令框架（純文字，不需 Markdown 語法或程式碼塊），內容請涵蓋以下三個面向，格式請使用條列式：

核心主題與文本類型：(例如：是一份「外星人參觀地球的動線說明書」、還是一段「中古世紀騎士的裝備清單」、或是一首「關於夏日午後的現代散文詩」？)
語氣與視角：(請搭配上述文本類型。例如：公家機關的生硬語氣、第一人稱的探險家口吻、或是浪漫主義詩人的口吻)
寫作手法與結構設計：(請完全拋棄八股文結構！如果是說明書，結構就是「條列步驟與警告事項」；如果是影評，結構就是「劇情簡介與主觀點評」；如果是海報，就是「斗大標題與時間地點」。請直接告訴寫手 AI 該如何切分這個特殊的版面！)`;
...
export async function generateQuestionsFromText(title, article, tags) {
    ...
    const examStyleInstruction = `【選項設計規範：極度重要，請模仿台灣國中教育會考】
        - **難度基準**：所有題目與選項的用語，必須符合 15 歲國中生的閱讀理解狀態。
        - **客觀絕對**：所有題目都必須有「唯一的客觀標準答案」。絕不可出現「你認為」、「你覺得最感動的是」等沒有標準答案的主觀問句。
        - **誘答力設計 (Distractors)**：錯誤選項（誘答選項）不能是荒謬或隨便湊數的。它們必須是「學生在閱讀時常常犯的錯誤」，例如：
            * 【過度推論】：文章只說Ａ會導致Ｂ，選項卻說Ａ一定會導致Ｃ。
            * 【斷章取義】：選項的敘述在文章某個角落有出現，但與該題問的重點無關。
            * 【因果反轉】：文章說主角因為生病所以沒去上學，選項卻說主角為了不去上學所以裝病。
            * 【以偏概全】：文章只說「部分」專家認為，選項卻寫「所有」專家皆證實。
        - **形式對稱**：同一題的四個選項，字數長度和句法結構應該盡可能相似，不要讓正確答案看起來特別長或特別短。正確答案位置也必須隨機。`;
...
2. ${examStyleInstruction}
3. **答題解析要求**：每題的 explanation 必須是寫給第一線老師備課看的。請清楚條列：
   - 【正解說明】：明確指出正解在原文第幾段、為何符合題意。
   - 【誘答分析】：逐一無情地剖析另外三個選項為什麼錯（例如：A 選項犯了某段的斷章取義錯誤、B 選項過度推論了作者的意思等）。
4. **輸出限制**：請嚴格返回 JSON 格式，不要包含 \`\`\`json 標籤外的任何廢話。`;
    const schema = {
        type: "OBJECT",
        properties: {
            questions: {
