...
export async function generateTopicIdea(topic, tags) {
    ...
    const prompt = `你是一位極具創意、幽默且知識淵博的「國文跨領域輔導團教師」。
你的任務是發想一個令國中生耳目一新、具備高度共鳴與思辨性的「閱讀測驗寫作架構 Prompt」。

主題方向：${topic ? `「${topic}」` : '請發揮創意，為國中生設計一個全新的素養考題主題。'}

**【極度重要的強制公式：跨領域知識點 + 青春期生活情境】**
為了打破死板無聊的作文題材，請你**必須**將一個「具體的理科或文史知識點」與「中學生的日常痛點或心理情境」進行結合。
*   範例 1：【物理學的慣性定律】 結合 【難以戒除的手遊成癮與拖延症】
*   範例 2：【生物學的互利共生與寄生】 結合 【班級小團體與人際邊界】
*   範例 3：【經濟學的沉沒成本】 結合 【對完美分數的執著與放棄的勇氣】
*   範例 4：【化學的催化劑】 結合 【校園裡引發群體同儕壓力的一個微小事件】
請每次都**隨機抽取**完全不同的領域知識（避免每次都寫一樣的學科），作為文章的隱喻或核心探討點。不但要貼近生活，還必須讓學生「學到一點課本外的冷知識或跨科概念」。

**【命題嚴格禁令】**
1. **嚴禁「數位分身/社群帳號/演算法」梗**：這些主題已經寫到爛了，絕對不准再出現「如果你的社群帳號有意識」、「演算法霸凌」、「數位人格」之類的設定！
2. **嚴禁老掉牙開場白**：絕對不允許出現「主題鎖定為...」、「主題設定為...」、「想像一下...」這類死板的機器人開場白。請直接用充滿張力或趣味的情境疑問句破題。
3. **拒絕生硬說教**：知識的帶入必須自然巧妙，不要像教科書一樣生硬，要像是個有趣的比喻。

${formatReq}${contentTypeReq}${difficultyReq}。

請直接回傳一份針對「寫手 AI」的指令框架（純文字，不需問候語、Markdown 語法或程式碼塊）。
內容請涵蓋以下三個面向，必須用激發寫作慾望的口吻，並以條列式輸出：
