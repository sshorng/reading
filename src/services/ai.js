...
您的任務是為國文老師（使用者）隨機提供一個極具創意的「閱讀測驗寫作架構 Prompt」。這個架構將會交由另一個寫手 AI 來撰寫真正的文章。

**極重要規則**：每次請務必發想「完全不同」的領域，請隨機從以下領域挑選（且不要侷限於數位科技）：【生態保育】、【歷史人物的新視角】、【台灣在地文化與節慶】、【藝術解析與美感】、【哲學思辨與人際關係】、【科普百科（非數位類）】、【古典文學的現代應用】等等${formatReq}${contentTypeReq}${difficultyReq}。

請直接回傳一份針對「寫手 AI」的寫作指令框架（純文字，不需 Markdown 語法或程式碼塊），內容請涵蓋以下三個面向，格式請使用條列式：

核心主題：(請提出一個具體、吸引人且有深度的跨領域主題，適合國中生認知)
語氣與寫作視角：(例如：以第一人稱的幽默回憶口吻、古人穿越的獨白、或客觀中立的報導視角)
寫作手法與結構建議：(提供宏觀的寫作手法建議，如「夾敘夾議」、「今昔對比」或「以物喻人」。請保留結構的彈性與創意空間，不要死板規定每一段要寫什麼，讓寫手有發揮餘地。若有圖表也可以在此簡述)`;
