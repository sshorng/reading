...
*   邏輯要清晰：論點 → 論據 → 小結，讀者不需要猜測你在說什麼。`
        : `**一、語言質感**
*   文字要「輕」，意思要「重」。用簡單的字說出複雜的事。
*   句子長短要錯落。不可連續出現三個以上結構相似的句子。
*   優先用具體的人、事、物、場景開頭，不要用抽象概念起筆。`;

    const stylePartThree = isAnalytical
        ? `**三、結構要求**
*   開頭：直接點明主題或提出核心問題，不需要故事性的場景鋪陳。
*   結尾：歸納論點或提出開放性的思考問題即可。**絕對禁止道德說教式的呼籲**（如「所以我們應該……」、「讓我們共同……」）；議論文的結尾是論點的收束，不是佈道。`
        : `**三、結構要求**
*   開頭：必須用一個具體的場景、故事片段或反常識的現象切入，絕不用「隨著……的發展」、「在當今社會……」起頭。
*   結尾：以餘韻收筆——可以是一個未解的疑問、一個細節的重返，或一個靜止的畫面。不可以是教訓或結論式的說教。`;
...
【提案格式要求】
直接回傳純文字提案，不要問候語、不要 Markdown、不要程式碼塊。條列以下三面向：

主題與核心概念：(用一個具體的場景、人物或反常識現象破題；說明這篇文章想帶讀者看見什麼)
文章形式與結構：(說明文體與行文節奏，例如：前段以故事切入，中段轉為分析，結尾以餘韻或懸念收筆)
寫作基調與禁令提醒：(說明文章的語氣與獨特視角；並明確指出這篇文章「最容易陷入的 AI 感陷阱」是什麼，要求寫手刻意迴避)
    `;
